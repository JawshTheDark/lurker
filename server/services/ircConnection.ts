    ...
    c.on('message', (event: Record<string, unknown>) => {
      ...
      if (isServer) target = `:server:${this.network.id}`;
        ...
        if (ctx) {
          ...
          // Fold to an existing buffer's casing (via the v1.1.2 buffers registry)
          // so a reply sourced as "ChanServ" doesn't fork history from a
          // "chanserv" buffer the user started (#289).
          const naturalTarget =
