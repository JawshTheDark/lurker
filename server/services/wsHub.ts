...
// bounded so a long disconnect can't produce an unbounded payload. Kept small
// because the connect snapshot reads this slice for EVERY online buffer
// synchronously on the event loop — on a large account (100+ buffers) on slow
// storage the old 500 turned reconnect into a multi-second stall that starved
// socket I/O and tripped IRC ping timeouts. The client lazily hydrates full
// history on buffer open, so a tighter cap only trims pre-loaded backlog.
const RESUME_GAP_CAP = 80;
// When the gap exceeds the cap we fall back to a fresh latest slice.
const RESUME_LATEST_LIMIT = 40;
